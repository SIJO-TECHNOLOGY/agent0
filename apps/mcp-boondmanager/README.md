...
| `getCandidateTechnicalDocument` | `CandidateTechnicalDocTool` | Retrieves the technical document (CV / skills profile) of a candidate via the candidate-scoped tab `GET /candidates/{candidateId}/technical-data` (BoondManager resolves the candidate → technical-document link server-side): title, skills text, experience level, training/diploma level, diplomas, expertise domains, activity sectors, tools with proficiency level, languages with level, and summary. `candidateId` echoes the requested candidate (`ID_PROFIL`); `id`/`tdId` are the technical document's own id (`ID_DT`) — a **distinct identifier**, never used as a candidate id and never the other way round. A candidate with no technical document yields an empty document (content fields null/empty). Call after `getCandidateDetail` for deep skills analysis. |
