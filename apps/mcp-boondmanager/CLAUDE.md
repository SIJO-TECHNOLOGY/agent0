...
| `getCandidateTechnicalDocument` | `CandidateTechnicalDocTool` | Retrieves the technical document (CV / skills profile) via the candidate-scoped tab `GET /candidates/{candidateId}/technical-data` (BoondManager resolves the candidate → technical-document link server-side): title, skills text, experience level, training/diploma level, diplomas, expertise domains, activity sectors, tools with proficiency level, languages with level, and summary. `candidateId` echoes the requested candidate (`ID_PROFIL`); `id`/`tdId` carry the technical document's own id (`ID_DT`), a **distinct** identifier — the candidate id is never reused as a technical-data id. A candidate without a technical document yields an empty document. Call after `getCandidateDetail` for deep skills analysis. |
