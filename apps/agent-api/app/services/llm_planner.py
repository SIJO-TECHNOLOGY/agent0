...
def build_planner_prompt(
    *,
    ...
    system = (
        ...
        "developer, business analyst, project manager) — and when the user "
        "restricts results to ONLY that métier ('uniquement des "
        "développeurs', 'only developers', 'que des devs', 'pas de business "
        "analysts'), ALSO set `role_exclusive` to \"true\" so candidates "
        "whose job title names a different métier are excluded instead of "
        "merely demoted —, `domain` or "
