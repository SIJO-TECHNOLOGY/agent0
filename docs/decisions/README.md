...
- [ADR-014 - Semantic CV Retrieval (RAG) As An Additive Recall Channel](./adr-014-cv-semantic-retrieval.md) - **built and measured, not adopted.** A vector index over CVs as a strictly additive recall channel; the A/B evaluation found no measurable benefit, because `searchCandidates` already full-text searches CV content.
